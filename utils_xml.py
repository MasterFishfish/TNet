...
def load_data_xml(ds_name):
    """
    """
    train_file = './dataset/%s/train.txt' % ds_name
    test_file = './dataset/%s/test.txt' % ds_name
    train_set = read_xml(path=train_file)
    test_set = read_xml(path=test_file)
    ...
    # sentence id
    # train_sid = np.arange(len(train_x)).astype('int32')
    # test_sid = np.arange(len(test_x)).astype('int32')
    # print(len(train_set))
    # print(len(test_set))
...
def get_func_words():
    ...
    with open('func_words.txt', encoding='utf-8') as fp:
        for line in fp:
            w = line.strip()
...
def get_embedding(vocab, ds_name, dim_w):
    """
    """
    if ds_name == '14semeval_laptop' or ds_name == '14semeval_rest':
        ...
        emb_file = '/projdata9/info_fil/lixin/Research/OTE/embeddings/glove_840B_300d.txt'
        pkl = './embeddings/%s_840B.pkl' % ds_name
...
def build_dataset_xml(ds_name, bs, dim_w, dim_func):
	"""
	"""
	dataset, vocab = load_data_xml(ds_name=ds_name)
